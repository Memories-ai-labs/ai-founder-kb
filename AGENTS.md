...
2. **No `reports/` folder.** Daily-crawl status notes belong in operator-only channels (Slack, ops dashboards, private gist), not in this public repo. If you find yourself wanting to write a status report into the repo, don't — write it elsewhere or just put it in the commit message. See **Daily crawl reporting** section below for where to send it.
...
## Daily crawl reporting

After every automated crawl run, **always** post a summary to Slack via the Slack MCP tool (`mcp__Slack__slack_send_message`). Do NOT write a report file to the repo.

- **Channel:** DM to Shawn — Slack user ID `U08QTUQLV19` (shawn.shen@memories.ai)
- **Send both:** a Slack message AND a `PushNotification` (phone banner)

**Slack message format:**

```
**AI Founder KB — Daily Crawl YYYY-MM-DD** 📡

**Stats:** sequoia=N | podcasts=N | blog-posts=N | news=N | total new files=N | pushed ✅/❌

---

**Top 3 stories today:**
[Three most signal-rich items with 2-3 sentence summaries each]

---

**Sources checked:** [list with ✅ / gaps noted]

**Repo:** `Memories-ai-labs/ai-founder-kb` @ `main` — N files total
```

**PushNotification message format:**
`DAILY YYYY-MM-DD: sequoia=N podcasts=N blog=N news=N files=N pushed=yes. [Top story one-liner].`
